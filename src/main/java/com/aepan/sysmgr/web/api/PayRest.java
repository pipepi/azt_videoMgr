...
import com._21cn.framework.entity.Email;
...
public class PayRest {
    ...
    public ModelAndView platformPay(
		...
		}else if(payType==PayConstants.TYPE_WECHAT||payType==PayConstants.TYPE_WECHATWAP){
...
//			sParaTemp.put("body", order.getProductId()+"dd");
	...
	private void wechatPay(HttpServletRequest request, ModelAndView mav,
		...
		if(codeImgUrl!=null&&!codeImgUrl.isEmpty()){
			...
			if(nowCal.getTime().before(createTime)){
				...
				if(payType==PayConstants.TYPE_WECHAT){
					...
					mav.setViewName("/paypage/request/wechatwap");
				}
		...
		try {
		    ...
		    if (entity != null) {
				...
				Map<String, String> rParaTemp = new HashMap<String, String>();
				rParaTemp.put("imgUrl", fileConfig.IMG_AZURE_PRE+rps.imgUrl); 
				rParaTemp.put("totalFee", order.getPrice()+"");
				if(payType==PayConstants.TYPE_WECHAT){
					mav.setViewName("/paypage/request/wechat");
				}else if(payType==PayConstants.TYPE_WECHATWAP){
	...
	private void notifyMessge(ProductOrder order) {
		String mobolie = order.getToMobile();
		String orderId=order.getOrderId();
		int userId=order.getSellerId();
		
		try {
			String c="尊敬的顾客"+mobolie+" ,您的订单（编号："+orderId+"）已经完成付款，请等待商家配送。如非本人操作请忽略该信息或联系客服：400-664-9000";
			SmsUtil.send(c, mobolie, configService,SmsConfig.TYPE_非验证码);
		} catch (Exception e) {
			// ignore
			logger.warn("Send sms failed:"+orderId+e.getMessage(),e);
		}
		
//		try {
//			
//			String mail = order.getToMail();
//			String var="{\"to\": [\""+mail+"\"],\"sub\":{\"%phoneNum%\": [\""+mobolie+"\"],\"%orderId%\":[\""+orderId+"\"]}}";
//			EmailUtil.sendMailTemplate(var, "user_finish_pay_notify", "[9库网-支付成功]", configService);
//		} catch (Exception e) {
//			// ignore
//			logger.warn("send email failed:"+orderId+e.getMessage(),e);
//		}
//		
		
		
		StringBuffer price = new StringBuffer().append(order.getPrice());
		
		if(PayConstants.TYPE_ALIPAY==order.getPayType()||PayConstants.TYPE_ALIPAY_WAP==order.getPayType()){
			price.append(" 支付宝支付");
		}else if(PayConstants.TYPE_WECHAT==order.getPayType()||PayConstants.TYPE_WECHATWAP==order.getPayType()){
			price.append(" 微信支付");
		}
		
		String createTime = DateUtil.getDateStr(order.getCreateTime());
		try {
		
			String mail = order.getToMail();
//			String var="{\"to\": [\""+mail+"\"],\"sub\":{\"%phoneNum%\": [\""+mobolie+"\"],\"%orderId%\":[\""+orderId+"\"]}}";
			String var = "{\"to\": [\""+mail+"\"],\"sub\":{\"%orderId%\":[\""+orderId+"\"],\"%productName%\": [\""+order.getProductNames()+"\"],\"%buyerName%\": [\""+order.getBuyersName()+"\"],\"%price%\": [\""+price+"\"],\"%createTime%\": [\""+createTime+"\"]}}";
			EmailUtil.sendMailTemplate(EmailUtil.TYPE_触发,var, "user_finish_pay_notify_html", "[9库网-支付成功]", configService);
		} catch (Exception e) {
			// ignore
			logger.warn("send email failed:"+orderId+e.getMessage(),e);
		}
		...
		try {
			...
			String var="{\"to\": [\""+mail+"\"],\"sub\":{\"%orderId%\":[\""+orderId+"\"]}}";
			EmailUtil.sendMailTemplate(EmailUtil.TYPE_触发,var, "notify_seller_order", "[9库网-发货通知]", configService);
		} catch (Exception e) {
			// ignore
			logger.warn("send email failed:"+orderId+e.getMessage(),e);
		}
...
//		}
	...
	private void paySuccess(ProductOrder order) {
		...
		if(ProductOrder.PAY_FOR_PRODUCT==order.getPayfor()){
			notifyMessge(order);

		}else if(ProductOrder.PAY_FOR_PACKAGE==order.getPayfor()){
