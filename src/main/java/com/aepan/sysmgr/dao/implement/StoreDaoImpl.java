...
public class StoreDaoImpl implements StoreDao {
	...
	public List<StoreSubInfo> getStoreSubInfoByIds(List<Integer> ids){
		StringBuffer sb = new StringBuffer();
		sb.append("select id,user_id,name,logo_url from t_sysmgr_store where id in (");
	...
	public void update(Store store){
        ...
        sb.append(" type=?, private_dns=?, com_address=?, com_tele=?,logo_url=?,status=?, update_time = ? where id = ? ");
	...
	public List<StoreSubInfo> getSellerOtherStores(int userId,int storeId){
		StringBuffer sb = new StringBuffer();
		sb.append("select id,user_id,name,logo_url from t_sysmgr_store where  user_id = ? and status=")
		.append(Store.STATUS_在线)
		.append("  and id <> ? order by id desc limit 0,5");
		return jdbcTemplate.query(sb.toString(), new Object[]{userId,storeId},STORESUBINFO_ROWMAPPER);
	}
	/**
	 * 获取同类型非本商家的播放器
	 * @param userId
	...
	public List<StoreSubInfo> getCategoryOtherStores(int userId,String type,int num){
		StringBuffer sb = new StringBuffer();
		sb.append("select id,user_id,name,logo_url from t_sysmgr_store where type like concat('%',?,'%') and status=")
		.append(Store.STATUS_在线)
		.append(" and user_id <> ? order by id desc limit 0,? ");
		return jdbcTemplate.query(sb.toString(), new Object[]{type,userId,num},STORESUBINFO_ROWMAPPER);
	}
}
