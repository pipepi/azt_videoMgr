               ...
               <ul class="bfq-listInfors">
                 <c:forEach items="${storelist }" var="store" varStatus="status">
                   ...
                   <div class="bf-edit"  style="width:460px;margin-left:80px;">
                     ...
                     <a href="javascript:clinkProductPage(${store.id})" class="bfq-cpBtns" id="link-product-a-${store.id}"  linked-product-ids = "${store.linkedProductIds==null?'':store.linkedProductIds }" >关联产品</a>
                     <a href="javascript:clinkVideoPage(${store.id})" class="bfq-spBtns" id="link-video-a-${store.id}" linked-video-ids = "${store.linkedVideoIds==null?'':store.linkedVideoIds }">关联视频</a>
