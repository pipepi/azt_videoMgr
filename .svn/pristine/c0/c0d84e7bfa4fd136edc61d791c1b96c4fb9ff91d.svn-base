...
<body>
            <div class="table-option clearfix">
                ...
                <div class="form-inline col-lg-offset-1">
			        ...
			        <i>
				        <span>状态：</span>
				        <select id="status" name="status" class="selected" style="width:150px;">
						    <option value="">请输入</option>
            ...
            <div class="table-responsive">
                <table class="table table-hover table-striped table-bordered" id="j-table">
                    ...
                    <tr>
                        ...
                        <th data-sorttype="pid">PID</th>
                        <th data-sorttype="url">资源路径</th>
                        <th data-sorttype="name">资源名称</th>
...
seajs.use(['bootstrap','$','underscore'],function(bootstrap,$,_) {
    ...
    function searchListPage() {
    	...
    	var status = $('#status').val().replace(/^\s*|\s*$/, "");
    	if(status!=null && status!=''){
    		queryParam += ('&eqStatus=' + encodeURIComponent(status));
    	}
        ...
        "aoColumns":[	//参数用来定义表格的列，这是一个数组
            ...
            },
            {
                ...
                "render": function(data){
                	var desc = data=='0'?'未审核':(data=='1'?'可用':'停用');
                	return desc;
                }
            },
            {
