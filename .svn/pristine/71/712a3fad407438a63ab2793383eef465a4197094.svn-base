...
function saveVideoLinks(){
	...
	//console.log(" _player = "+_player+"   vids = "+vids);
	$.post(url,{storeId:_player,videoIds:vids},function(data){
		 var datas = data.substring(data.indexOf('{')+1,data.lastIndexOf('}'));
		  ...
		  if(attr.success){
			  ...
			  console.log(attr.storeLogo);
			  if(attr.storeLogo!=null&&attr.storeLogo.length>0){
				  console.log("1111111111");
				  $('#store-logo-'+_player).attr("src",_imgpre+attr.storeLogo);
			  }else{
				  console.log("22222222");
				  $('#store-logo-'+_player).attr("src",_ctx+"/source/images/aztimg/logo.jpg");
			  }
			 $("#video-list").toggle();
		  }else{
